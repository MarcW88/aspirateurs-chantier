(() => {
  ...
  if (!body.classList.contains('comparison-page')) return;

  const burger = document.querySelector('.burger');
  const nav = document.querySelector('.site-nav');

  if (burger && nav) {
    if (!nav.id) nav.id = 'comparison-site-nav';
    ...
    nav.addEventListener('click', event => {
      if (event.target.closest('a[href]') && window.matchMedia('(max-width: 768px)').matches) setOpen(false);
    });
  ...
  const sidebarCta = sidebar?.querySelector('.btn-primary, .btn-accent');
